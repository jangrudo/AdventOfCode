...
pip install --editable /path/to/AdventOfCode/aoc-shortcuts
```
...
`aoc-shortcuts` directory to `PYTHONPATH`.

To resolve any package dependencies that might have been recently added to
`pyproject.toml`, run:

```
pip install --upgrade --editable /path/to/AdventOfCode/aoc-shortcuts
```
